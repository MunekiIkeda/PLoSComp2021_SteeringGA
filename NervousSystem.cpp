...
void NervousSystem::EulerStep(double stepsize)
{
    ...
    for (int i = 1; i <= size; i++) {
		...
		for (int j = 1; j <= NumElectricalConns[i]; j++) {
			...
			if (paststates[electricalweights[i][j].from] - paststates[electricalweights[i][j].from] != 0) {
				cout << i << "-" << electricalweights[i][j].from << ": " << paststates[electricalweights[i][j].from] <<endl;
			}
		}
        // Take the step
